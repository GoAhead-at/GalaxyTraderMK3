...
function gtMenu.onDataUpdate()
    ...
    if menu and menu.infoTableMode == "galaxytraderinfo" then
        ...
        local timeSinceLastRefresh = currentTime - gtMenu.lastRefreshTime
        
        -- Throttle refreshes to prevent disorienting row jumping
        if timeSinceLastRefresh < gtMenu.refreshCooldown then
            -- DebugError(string.format("[GT Menu] Data updated but skipping refresh (%.1fs since last, cooldown=%.1fs)", 
            --     timeSinceLastRefresh, gtMenu.refreshCooldown))
            return
        end
        
        -- DebugError("[GT Menu] Data updated - auto-refreshing display")
        gtMenu.lastRefreshTime = currentTime
...
function gtMenu.displayPilotTable(frame, instance, yOffset)
    ...
    local headerRow = pilotTable:addRow(true, { bgColor = Color["row_title_background"], fixed = true })
    ...
    for i, pilot in ipairs(pilotsToDisplay) do
        ...
        -- Create selectable row (first param 'true' makes it selectable)
        local row = pilotTable:addRow(true, {})
    ...
    if selectedRowIndex then
        pilotTable:setSelectedRow(selectedRowIndex)
        -- DebugError(string.format("[GT Menu] Restored selection to row %d (Ship: %s)", selectedRowIndex, gtMenu.selectedShipId))
    end
...
function gtMenu.displayRenaming(frame, instance, yOffset)
    ...
    -- Header row
    local headerRow = renamingTable:addRow(true, { bgColor = Color["row_title_background"], fixed = true })
    ...
    for i, pilot in ipairs(pilotsToDisplay) do
        ...
        local row = renamingTable:addRow(true, { interactive = true })
...
function gtMenu.displayPilotControl(frame, instance, yOffset)
    ...
    -- Header row
    local headerRow = controlTable:addRow(true, { bgColor = Color["row_title_background"], fixed = true })
    headerRow[1]:createText(ReadText(77000, 8100), { halign = "center" })  -- Ship ID
    headerRow[2]:createText(ReadText(77000, 8101), { halign = "center" })  -- Pilot
    ...
    for i, pilot in ipairs(pilotsToDisplay) do
        ...
        -- Use interactive = true to allow buttons in the row
        local row = controlTable:addRow(true, { interactive = true })  -- interactive = true allows buttons!
    ...
    -- Restore selection in X4 table system (preserves selection across refreshes/tab switches)
    if selectedRowIndex then
        controlTable:setSelectedRow(selectedRowIndex)
        -- DebugError(string.format("[GT Menu] Control tab: Restored selection to row %d", selectedRowIndex))
    end

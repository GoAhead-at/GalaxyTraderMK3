...
if not Helper or not Helper.playerInfoConfigTextRight then
    DebugError("[GT CreditsDue] Helper.playerInfoConfigTextRight not found - override not installed")
    return
end
...
local function pollBridgeData()
    ...
    if (state.lastLoggedCredits ~= state.projectedCredits) or (state.lastLoggedEnabled ~= state.enabled) then
        state.lastLoggedCredits = state.projectedCredits
        state.lastLoggedEnabled = state.enabled
        DebugError("[GT CreditsDue Bridge] enabled=" .. tostring(state.enabled) ..
            " credits=" .. tostring(state.projectedCredits) ..
            " format=" .. tostring(state.numberFormat) ..
            " useMSuffix=" .. tostring(state.useMSuffix) ..
            " threshold=" .. tostring(state.mSuffixThreshold))
    end
end

-- =============================================================================
-- PLAYER INFO TEXT HOOK (LEFT SIDE, LINE 3)
-- =============================================================================

-- Vanilla left text has 3 lines: playername \n gametime \n playermoney.
-- Append compact GT due text to line 3 so it appears directly after money.
...
Helper.playerInfoConfigTextLeft = function(cell, width, ismultiverse)
    ...
    lines[3] = (lines[3] or "") .. gtToken

    -- Keep vanilla behavior: respect provided width budget for the left text block.
    if width and width > 0 then
        lines[3] = TruncateText(lines[3], Helper.playerInfoConfig.fontname, Helper.playerInfoConfig.fontsize, width)
    end

    return table.concat({ lines[1] or "", lines[2] or "", lines[3] or "" }, "\n")
end

DebugError("[GT CreditsDue] playerInfoConfigTextLeft hook installed (inline money approach)")
